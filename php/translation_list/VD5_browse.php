...
function getList($searchArr,$page,$count,$database){
	    ...
	    if($database=='VariantDesc'){
		    ...
		    $variantdesc_query="SELECT e.LastName as TranslatorLastName,e.MiddleName as TranslatorMiddleName,
		    e.FirstName as TranslatorFirstName,ee.LastName as ReviewerLastName,ee.MiddleName as ReviewerMiddleName,
		    ee.FirstName as ReviewerFirstName,
		    VariantDesc.VariantDesc_en,VariantDesc.CreateDate,VariantDesc.VariantDescID,Gene.Symbol,Variant.HGVSp,VariantDesc.Count,VariantDesc.Status,VariantDesc.TranslationDate FROM VariantDesc JOIN Variant ON VariantDesc.VariantID=Variant.VariantID JOIN Transcript ON Transcript.TranscriptID=Variant.TranscriptID JOIN Gene ON Gene.GeneID=Transcript.GeneID join Employee as e
		    	on e.EmployeeID= VariantDesc.TranslatorID
		    	join Employee as ee
		    	on ee.EmployeeID=VariantDesc.ReviewerID".$where.$whereSql." limit ".$start.",". $count;
		    ...
		    $VariantDesc_total_search_query="SELECT count(*) as total FROM VariantDesc JOIN Variant ON VariantDesc.VariantID=Variant.VariantID JOIN Transcript ON Transcript.TranscriptID=Variant.TranscriptID JOIN Gene ON Gene.GeneID=Transcript.GeneID join Employee as e
		    	on e.EmployeeID= VariantDesc.TranslatorID
		    	join Employee as ee
		    	on ee.EmployeeID=VariantDesc.ReviewerID".$where.$whereSql;
		...
		//** query GeneIntro table
	        $GeneIntro_query="SELECT e.LastName as TranslatorLastName,e.MiddleName as TranslatorMiddleName,
		    e.FirstName as TranslatorFirstName,ee.LastName as ReviewerLastName,ee.MiddleName as ReviewerMiddleName,
		    ee.FirstName as ReviewerFirstName,GeneIntro.GeneIntr_en,GeneIntro.CreateDate,GeneIntro.GeneIntroID,Gene.Symbol,GeneIntro.Count,GeneIntro.Status,GeneIntro.TranslationDate FROM GeneIntro JOIN Gene ON GeneIntro.GeneID=Gene.GeneID join Employee as e
		    	on e.EmployeeID= GeneIntro.TranslatorID
		    	join Employee as ee
		    	on ee.EmployeeID=GeneIntro.ReviewerID".$where.$whereSql." limit ".$start.",". $count;
		...
		while($row = $result->fetch_assoc()) {
	        ...
	        $GeneIntro_total_search_query="SELECT count(*) as total FROM GeneIntro JOIN Gene ON GeneIntro.GeneID=Gene.GeneID join Employee as e
		    	on e.EmployeeID= GeneIntro.TranslatorID
		    	join Employee as ee
		    	on ee.EmployeeID=GeneIntro.ReviewerID".$where.$whereSql;
		...
		//** query GeneIntro table
	        $Therapy_Test_query="select e.LastName as TranslatorLastName,e.MiddleName as TranslatorMiddleName,
		    e.FirstName as TranslatorFirstName,ee.LastName as ReviewerLastName,ee.MiddleName as ReviewerMiddleName,
		    ee.FirstName as ReviewerFirstName,te.TherapyID,te.Keyword,te.TherapyEfficacy_en,te.`Status`,te.CreateDate,te.Count,te.TranslationDate from TherapyEfficacy as te join Employee as e
		    	on e.EmployeeID= te.TranslatorID
		    	join Employee as ee
		    	on ee.EmployeeID=te.ReviewerID
".$where.$whereSql." limit ".$start.",". $count;
		...
		while($row = $result->fetch_assoc()) {
	        ...
	        $Therapy_Test_total_search_query="select count(TherapyID) as total from TherapyEfficacy as te join Employee as e
		    	on e.EmployeeID= te.TranslatorID
		    	join Employee as ee
		    	on ee.EmployeeID=te.ReviewerID
".$where.$whereSql;
